CREATE FUNCTION `create_term`(_name TEXT, _slug TEXT, _taxonomy TEXT, _parent BIGINT(20)) -- _taxonomy: product_cat, product_tag
RETURNS BIGINT(20)
BEGIN
    SET @id_term = (SELECT T.`term_id` FROM `wp_terms` as T INNER JOIN `wp_term_taxonomy` as TX ON T.term_id = TX.term_id WHERE LOWER(T.`name`) = LOWER(_name) AND TX.parent = _parent AND TX.taxonomy = _taxonomy LIMIT 1);
